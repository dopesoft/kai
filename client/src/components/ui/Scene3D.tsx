...
const Box = ({ position, rotation }: { position: [number, number, number], rotation: [number, number, number] }) => {
    ...
    // Create the exact shape from user's code
...
const AnimatedBoxes = ({ count = 50 }: { count?: number }) => {
    ...
    const boxes = React.useMemo(() => Array.from({ length: count }, (_, index) => ({
    ...
    return (
        <div className="w-full h-full z-0">
            <Canvas camera={{ position: cameraPosition, fov: 40 }}>
                ...
                <AnimatedBoxes count={compact ? 30 : 50} />
            </Canvas>
        </div>
    );
}; 